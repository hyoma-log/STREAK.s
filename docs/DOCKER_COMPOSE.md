...
- db: `postgres:16`。環境変数で初期 DB とユーザをセットし、永続化のため `db_data` ボリュームを使用します。
...
上記は `docker-compose.yml` の `db` サービスで設定されています。Drupal のインストール時には、DB ホストに `db`（サービス名）を指定してください。
...
3. Drupal のインストール画面が表示されたら、以下の手順で進めます：
   - 言語を選択（日本語または英語）
   - プロファイルを選択（Standard を推奨）
   - データベース設定画面で以下の情報を入力：
     - **データベースタイプ**: PostgreSQL
     - **データベース名**: `streak`
     - **データベースユーザー名**: `postgres`
     - **データベースパスワード**: `postgres`
     - **データベースホスト**: `db`（重要：`localhost`ではなく、docker-compose.ymlで定義したサービス名`db`を使用）
     - **データベースポート**: `5432`（デフォルト）
   - サイト情報を入力（サイト名、管理者アカウント情報など）
   - インストール完了後、管理者アカウントでログインできることを確認

4. インストールが完了したら、`http://localhost:8080/user/login` でログインできます。
